...
async def get_learned_rules(request: Request):
    """Return the persistent list of rules auto-generated from LLM scans."""
    _check_auth(request)
    import json as _json
    import os as _os
    path = _os.environ.get("MAILMITE_LEARNED_RULES") or \
           _os.path.expanduser("~/.mailmite/learned_rules.json")
    if not _os.path.exists(path):
        return {"path": path, "count": 0, "rules": []}
    try:
        with open(path, "r") as fh:
            data = _json.load(fh)
        rules = data.get("rules", [])
        return {"path": path, "count": len(rules), "rules": rules}
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Could not read learned rules: {exc}")
...
async def _ensure_report_fresh(scan_id: str, report_path: Path, detail) -> None:
    """Regenerate the on-disk report if SQLite triage state is newer."""
    if detail.state != "done":
        return
    ...
    # If no triage update or report is fresher than latest triage, no rebuild needed
    if latest == 0:
        return
    if report_path.exists() and (report_path.stat().st_mtime * 1000) >= latest:
        return
