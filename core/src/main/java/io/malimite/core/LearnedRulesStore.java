 ...
 * <p>Paths:
 * <ul>
 *   <li>iOS: {@code $MALIMITE_LEARNED_RULES_IOS} or {@code $MALIMITE_LEARNED_RULES}
 *       or {@code ~/.malimite/learned_rules.json}</li>
 *   <li>Android: {@code $MALIMITE_LEARNED_RULES_ANDROID}
 *       or {@code ~/.malimite/learned_rules_android.json}</li>
 * </ul>
 *
 * <p>If the new {@code ~/.malimite/} file is missing, falls back to the pre-rename
 * {@code ~/.mailmite/} path when that file exists (read-only discovery; writes go to
 * whichever path was resolved).
 *
...
public final class LearnedRulesStore {
    ...
    public static Path defaultPath(PackagePlatform platform) {
        if (platform == PackagePlatform.ANDROID) {
            String env = firstNonBlank(
                    System.getenv("MALIMITE_LEARNED_RULES_ANDROID"),
                    System.getenv("MAILMITE_LEARNED_RULES_ANDROID"));
            if (env != null) return Path.of(env);
            String home = System.getProperty("user.home", "/tmp");
            return preferExisting(
                    Path.of(home, ".malimite", "learned_rules_android.json"),
                    Path.of(home, ".mailmite", "learned_rules_android.json"));
        }
        String iosEnv = firstNonBlank(
                System.getenv("MALIMITE_LEARNED_RULES_IOS"),
                System.getenv("MAILMITE_LEARNED_RULES_IOS"));
        if (iosEnv != null) return Path.of(iosEnv);
        String legacy = firstNonBlank(
                System.getenv("MALIMITE_LEARNED_RULES"),
                System.getenv("MAILMITE_LEARNED_RULES"));
        if (legacy != null) return Path.of(legacy);
        String home = System.getProperty("user.home", "/tmp");
        return preferExisting(
                Path.of(home, ".malimite", "learned_rules.json"),
                Path.of(home, ".mailmite", "learned_rules.json"));
    }
    ...
    /** Prefer {@code preferred} unless missing while {@code fallback} exists. */
    private static Path preferExisting(Path preferred, Path fallback) {
        if (Files.exists(preferred) || !Files.exists(fallback)) return preferred;
        log.info("LearnedRulesStore: using legacy path {} ({} missing)", fallback, preferred);
        return fallback;
    }
